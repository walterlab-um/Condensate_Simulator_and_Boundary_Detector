...
df_groundtruth = pd.DataFrame(
    {
        ...
        "y_nm": center_y,
        "r_nm": condensate_r,
        "C_dilute": np.repeat(C_dilute, N_fov * N_condensate),
        "C_condensed": np.repeat(C_condensed, N_fov * N_condensate),
...
for current_fov in track(index):
    ...
    #################################################
    # Step 2: Ground truth high-resolution image baesd on a spherecal volume projection model
