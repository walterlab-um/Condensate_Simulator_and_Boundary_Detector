...
def rmsd_d2edge(df):
    ...
    rmsd_d2edge = np.sqrt(np.mean(current_d2edge**2))
...
def compute_chunck(df, tag):
    ...
    for i in range(30):
        ...
        df_ref = df_truth.iloc[i * 100 : (i + 1) * 100 - 1]
        rmsd_distance2edge.append(rmsd_d2edge(df_current))
        rmsd_area_nm2.append(np.sqrt(np.mean(df_ref["rmsd_area_nm2"] ** 2)))
        rmsd_aspect_ratio.append(np.sqrt(np.mean(df_ref["rmsd_aspect_ratio"] ** 2)))
        rmsd_distance2center.append(
            np.sqrt(np.mean(df_ref["rmsd_distance2center"] ** 2))
        )
    df_new = pd.DataFrame(
        {
...
# for method 2
df_canny = pd.read_csv(path_threshold)
...
# for method 3
df_blob = pd.read_csv(path_threshold)
